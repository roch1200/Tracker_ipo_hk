...
def test_ipo_model():
    ...
    print("=" * 70)
    print("🔬 TEST 1: Model IPO - Càlculs de rendibilitat")
    print("=" * 70)
    ...
    print("✅ IPO amb GUANY: Tots els càlculs correctes")
    ...
    print("\n✅ IPO amb PÈRDUA: Tots els càlculs correctes")
    ...
    print("\n✅ IPO PROPERA (sense preus): Returns = None (correcte)")
    ...
    try:
        ...
        print("✅ EDGE CASE (preu IPO=0): Return = None (correcte)")
    except ZeroDivisionError:
        print("❌ ERROR: Divisió per zero detectada!")
        return False
    
    print("\n✅ TOTS ELS TESTS DEL MODEL PASSATS!")
    return True
...
def test_summary():
    ...
    print("\n" + "=" * 70)
    print("📊 TEST 2: Resum Estadístic (IPOSummary)")
    print("=" * 70)
    
    ipos = create_demo_ipos()
    summary = IPOSummary(ipos)
    
    print(f"✅ Total IPOs passades: {summary.total_ipos}")
    print(f"✅ Properes IPOs: {summary.upcoming_ipos}")
    print(f"✅ Positives 1r dia: {summary.positive_first_day}")
    print(f"✅ Negatives 1r dia: {summary.negative_first_day}")
    print(f"✅ Rendiment mitjà 1r dia: {summary.average_first_day_return:+.2f}%")
    print(f"✅ Rendiment mitjà 1a setmana: {summary.average_first_week_return:+.2f}%")
    print(f"✅ Capital total recaptat: HK${summary.total_funds_raised:,.0f}M")
    print(f"✅ Millor performer: {summary.best_performer[1]} ({summary.best_performer[0]:+.2f}%)")
    print(f"✅ Pitjor performer: {summary.worst_performer[1]} ({summary.worst_performer[0]:+.2f}%)")
    print(f"✅ IPOs per sector: {summary.ipo_by_sector}")
    ...
    print("\n✅ TOTS ELS TESTS DEL SUMMARY PASSATS!")
    return True
...
def test_returns_detail():
    ...
    print("\n" + "=" * 70)
    print("📋 TEST 3: Detall de rendibilitat per IPO")
    print("=" * 70)
...
def test_market_stats():
    ...
    print("\n" + "=" * 70)
    print("📈 TEST 4: Dades reals de HKEX (Abril 2026)")
    print("=" * 70)
    ...
    print(f"✅ Mes: {hkex_data['month']}")
    print(f"✅ Empreses llistades: {hkex_data['total_listed_companies']:,}")
    print(f"✅ Noves IPOs al mes: {hkex_data['new_listings_month']}")
    print(f"✅ Noves IPOs al 2025: {hkex_data['new_listings_year']}")
    print(f"✅ Capitalització total: HK${hkex_data['total_market_cap']:,.1f} Bn")
    print(f"✅ Volum mitjà diari: HK${hkex_data['avg_daily_turnover']:,.0f} Mn")
...
def test_edge_cases():
    ...
    print("\n" + "=" * 70)
    print("⚠️  TEST 5: Casos límit (Edge Cases)")
    print("=" * 70)
    ...
    if ipo_null.first_day_return is None:
        print("✅ IPO sense dades de preu: Returns = None (correcte)")
    else:
        print("❌ ERROR: IPO sense dades hauria de tenir returns = None")
        errors += 1
    ...
    if ipo_high.first_day_return == 100.0:
        print(f"✅ IPO preu alt (+100%): Càlcul correcte")
    else:
        print(f"❌ ERROR: Esperat +100%, obtingut {ipo_high.first_day_return}%")
        errors += 1
    ...
    if ipo_zero.total_return == -100.0:
        print(f"✅ IPO val 0 (-100%): Càlcul correcte")
    else:
        print(f"❌ ERROR: Esperat -100%, obtingut {ipo_zero.total_return}%")
        errors += 1
    ...
    if summary_many.total_ipos == 100:
        print(f"\n✅ STRESS TEST (100 IPOs): Resum generat correctament")
        ...
        print(f"❌ STRESS TEST: Esperades 100 IPOs, tenim {summary_many.total_ipos}")
        errors += 1
    
    if errors == 0:
        print("\n✅ TOTS ELS EDGE CASES PASSATS!")
    else:
        print(f"\n❌ {errors} ERROR(S) TROBAT(S)!")
...
def main():
    ...
    for name, test_fn in tests:
        try:
            ...
            print(f"\n❌ ERROR inesperat a '{name}': {e}")
            failed += 1
    
    print("\n" + "=" * 70)
    print(f"📊 RESUM FINAL: {passed} tests passats, {failed} tests fallats")
    print("=" * 70)
    ...
    if failed == 0:
        ...
        print(f"\n⚠️  {failed} test(s) fallat(s). Cal revisar.")
